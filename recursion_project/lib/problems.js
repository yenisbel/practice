...
function sumArray(array) {
    ...
    let suma = array[0];
    return suma + sumArray(array.slice(1))
...
function flatten(data) {
    ...
    let resultFlatten = [];
    data.forEach(ele =>{
        let eleFlatten = flatten(ele);
        resultFlatten.push(...eleFlatten);
    })
...
function pathFinder(directories, targetFile) {
    for (let key in directories) {
        ...
        let pathComplete = pathFinder(directories[key], targetFile);
        if(pathComplete !== null){
            return `${key}${pathComplete}` 
        }
